...
def _get_rsi(yahoo_sym, fyers_sym):
    ...
    try:
        import yfinance as yf
        hist = yf.Ticker(yahoo_sym).history(period="10d", interval="15m")
        if not hist.empty:
    ...
    if not prices and fyers_sym:
        try:
            ...
            resp  = fyers.history(data={
                "symbol": fyers_sym, "resolution": "15", "date_format": "1",
                "range_from": (today - timedelta(days=10)).strftime("%Y-%m-%d"),
...
# =========================
def _detect_candle_pattern(yahoo_sym, fyers_sym):
    """Detect last candle pattern from 15min data"""
    try:
        import yfinance as yf
        hist = yf.Ticker(yahoo_sym).history(period="2d", interval="15m")
        ...
        elif lower_wick >= body * 2 and upper_wick <= body * 0.5 and cl >= o:
            pattern = "Hammer 🔨 (Bullish)"
            signal  = "bullish"

        # Shooting Star — small body at bottom, long upper wick (bearish)
        elif upper_wick >= body * 2 and lower_wick <= body * 0.5 and cl <= o:
            pattern = "Shooting Star ⭐ (Bearish)"
            signal  = "bearish"

        # Bullish Engulfing
        elif (cl > o and                          # Current green
              float(c2["Close"]) < float(c2["Open"]) and  # Prev red
              o < float(c2["Close"]) and
              cl > float(c2["Open"])):
            pattern = "Bullish Engulfing 🟢"
            signal  = "bullish"

        # Bearish Engulfing
        elif (cl < o and                          # Current red
              float(c2["Close"]) > float(c2["Open"]) and  # Prev green
              o > float(c2["Close"]) and
              cl < float(c2["Open"])):
        ...
        if pattern:
            return {"pattern": pattern, "signal": signal, "o": round(o,2), "h": round(h,2), "l": round(l,2), "c": round(cl,2)}
        return {"pattern": "No clear pattern ⚪", "signal": "neutral", "o": round(o,2), "h": round(h,2), "l": round(l,2), "c": round(cl,2)}
...
# MASTER ENGINE
# =========================
def master_engine(market, oc_data, greeks, atm, expiry_str, zones, rsi, vix_data, asset_key, ema_data=None, fib_data=None, candle=None):
    asset      = ASSETS[asset_key]
    price      = market["price"]
    ...
    if candle and candle.get("signal") != "neutral":
        if candle["signal"] == "bullish":
            signals["Candle"] = f"{candle['pattern']}"; sb += 1
        elif candle["signal"] == "bearish":
            signals["Candle"] = f"{candle['pattern']}"; sb_bear += 1
    elif candle:
        signals["Candle"] = f"{candle['pattern']}"
    else:
        signals["Candle"] = "N/A"
    ...
    return {
        ...
        "ema_data": ema_data, "fib_data": fib_data, "candle": candle,
    }
...
def format_output(res, market, oc_data, zones, rsi, rsi_sig, vix_data, asset_key):
    ...
    candle_data = res.get("candle")
    if candle_data and candle_data.get("pattern"):
        candle_str = f"{candle_data['pattern']} | O:`{candle_data.get('o','?')}` H:`{candle_data.get('h','?')}` L:`{candle_data.get('l','?')}` C:`{candle_data.get('c','?')}`"
    else:
        candle_str = "Data unavailable"

    msg = f"""📊 **{asset['name']} — Options Analysis (15m)**
...
*📢 Sirf educational purpose ke liye | SEBI registered nahi hain | Apne vivek se trade karein •* {datetime.now().strftime("%d %b %H:%M")}"""
...
━━━━━━━━━━━━━━━━
"""
    msg += "*📢 Sirf educational purpose ke liye | SEBI registered nahi hain | Apne vivek se trade karein*"
    return msg
...
async def handle_trade(message, asset_key):
    ...
    # Candle pattern
    candle = await run_in_thread(_detect_candle_pattern, asset.get("yahoo_symbol", "^NSEI"), asset.get("fyers_symbol", ""), timeout=12)
    ...
    zones = get_zones(market)
    res   = master_engine(market, oc_data, greeks, atm, expiry_str, zones, rsi, vix_data, asset_key, ema_data, fib_data, candle)
